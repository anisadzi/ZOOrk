...
void Passage::createBasicPassage(Room* from, Room* to, const std::string &direction, bool bidirectional, bool locked, const std::string& keyItem) {
...
Passage::Passage(const std::string &n, const std::string &d, Room* from, Room* to)
    : Location(n, d), fromRoom(from), toRoom(to), locked(false) {
...
Passage::Passage(const std::string &n, const std::string &d, std::shared_ptr<Command> c, Room* from, Room* to)
    : Location(n, d, std::move(c)), fromRoom(from), toRoom(to), locked(false) {}
...
const std::string& Passage::getKeyItem() const {
    return keyItem;
}

bool Passage::tryUnlock(Player& player) {
    if (!locked) {
        return true;
    }
